...
Consumer and fork projects that have `quadrants:` in their `data/config.json` and `quadrant:` in their markdown frontmatter must continue to work. We implement a runtime shim at two boundaries:
...
- Consumer forks continue to work without changes. They see one-time `consola.warn` lines at build time.
...
- **Rename without shim, hard break.** Rejected: This would silently break every fork's `data/config.json`. Unacceptable for a published npm package consumed by external repos.
- **Compile-time codemod for forks.** Rejected: This adds too much complexity for a one-time vocabulary change. A runtime warn-and-alias is simpler and self-documenting.