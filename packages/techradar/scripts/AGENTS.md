...
| `secretsScan.ts` | Shared trufflehog plumbing used by `preCommitSecrets.ts` and `checkSecrets.ts`: cached `execa` import, `TrufflehogMissingError`, tmpdir-traversal-safe `getMirroredPath`, worktree detection (`isGitWorktree`), and `runTrufflehogFilesystem`. Not invoked directly. |
| `preCommitSecrets.ts` | Pre-commit secret gate. Materializes staged blobs to a tmpdir mirror via `git show :<path>` and runs `trufflehog filesystem`. Warns and exits 0 if trufflehog is missing (lenient — devs may not have it installed yet). Run via `pnpm run precommit:secrets`. |
| `checkSecrets.ts` | DoD secret gate. Changes cwd to the repo top-level; in a regular checkout runs `trufflehog git file://.` against history; in a `git worktree` runs filesystem mode against a tmpdir mirror of `git ls-files` (workaround for `trufflesecurity/trufflehog#4553` — see ADR-0011). Fails hard if trufflehog is missing (strict — required by DoD). Invoked via root `pnpm run check:sec:secrets`, which delegates here with `pnpm --filter`. |
