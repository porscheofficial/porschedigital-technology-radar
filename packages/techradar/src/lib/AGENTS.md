...
- **No Node.js built-ins.** `src/` is client-side code — `node:fs`, `node:path`, `node:child_process`, etc. crash in the browser. Packages that transitively import them (e.g. `consola`, `chalk`) are equally banned. Use `console.warn`/`console.error` instead of logger libraries. (Checked: `.dependency-cruiser.cjs` → `no-node-builtins-in-src`.)
