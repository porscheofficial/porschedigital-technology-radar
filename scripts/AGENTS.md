...
| `validateFrontmatter.ts` | **The Zod schema for radar item frontmatter.** Single source of truth shared by `buildData.ts` and `npm run validate`. |
...
| `checkConfigReadmeSync.ts` | Asserts that every leaf key in `data/config.default.json` and every Zod field in `validateFrontmatter.ts` is documented in `README.md`. Run via `npm run check:arch:readme`. |
...
- **Keep `validateFrontmatter.ts` and `data/config.default.json` in sync with `README.md`.** Any field add/rename/removal must also update the README's Configuration tables and/or Front-matter attributes table. (Checked: `npm run check:arch:readme`.)
