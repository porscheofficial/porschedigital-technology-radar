...
- **Every page route must produce a file in `out/`.** New routes require updating `scripts/checkBuildOutput.ts` if they aren't covered by the data-driven loop (statics, segments, items). (Checked: `pnpm run check:build:routes` after `pnpm run build`.)
